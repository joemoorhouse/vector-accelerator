...
namespace VectorAccelerator
{
    public class Assertions
    {
        ...
        {
            if (a.Rows != b.Rows || a.Columns != b.Columns) throw new ArgumentException("shape mismatch");
        }
        
        public static void AssertIsRow(NArray row, string paramName)
        {
            if (row.Rows != 1) throw new ArgumentException("is not a row", paramName);
        }
        ...
        {
            if (column.Columns != 1) throw new ArgumentException("is not a column", paramName);
        }
        ...
        public static void AssertRowMatchesMatrix(NArray matrix, NArray row, string matrixParam, string rowParam)
        {
            ...
            if (row.Columns != matrix.Columns) throw new ArgumentException(string.Format("mismatching row {0} and matrix {1}", rowParam, matrixParam));
        }
