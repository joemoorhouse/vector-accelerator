...
namespace VectorAccelerator.DeferredExecution.Expressions
{
            ...
            for (int i = 0; i < chunkCount; ++i)
            {
                ...
                for (int j = 0; j < block.Operations.Count; ++j)
                {
                    ...
                    if (operation.Type == typeof(NArray))
                    {
                        ...
                        {
                            var arrayToReturn = (item.Storage as ChunkyStorage<double>).GetChunk(i);
    ...
    public class OrderedOutputs<T>
    {
        ...
        public OrderedOutputs(NArray<T>[] outputs, int[] outputsIndices)
        {
