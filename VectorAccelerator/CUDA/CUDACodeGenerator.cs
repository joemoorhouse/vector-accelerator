...
namespace VectorAcceleration
{
    class CUDACodeGenerator
    {
        ...
        public void GenerateCode(BlockExpression block)
        {
        ...
        public Expression Visit(Expression exp, StringBuilder builder)
        {
            ...
            switch (exp.NodeType)
            {
                case ExpressionType.Block:
                    return VisitBlock((BlockExpression)exp, builder);
                ...
                case ExpressionType.Parameter:
                    return VisitParameter((ParameterExpression)exp, builder);
                case ExpressionType.MemberAccess:
                case ExpressionType.Call:
                    return VisitMethodCall((MethodCallExpression)exp, builder);
        ...
        private Expression VisitBlock(BlockExpression block, StringBuilder builder)
        {
            _code.Append(prototype_vectorOperation);

            var variables = block.Variables.
                Where(b => VariableIsParameter(b));

            var locals = block.Variables.
                Where(b => !VariableIsParameter(b));

            //builder.
            SetKernelArguments(variables.Select(v => v.Name));
            SetKernelLocals(locals.Select(l => l.Name));
        ...
        protected Expression VisitBinary(BinaryExpression binary, StringBuilder builder)
        {
            ...
            //WriteExpression(binary, builder);
        ...
        protected Expression VisitMethodCall(MethodCallExpression call, StringBuilder builder)
        {
        ...
        protected Expression VisitParameter(ParameterExpression variable, StringBuilder builder)
        {
        ...
        {
            //WriteExpression(binary.Left, builder);
            Visit(binary.Left, builder);
            WriteEquals(builder);
            Visit(binary.Right, builder);
