...
{    
    class AlgorithmicDifferentiator
    {
            ...
            params VectorParameterExpression[] independentVariables)
        {
            ...
            dxNdx[N] = new ConstantExpression(1.0); 
            for (int i = N - 1; i >= 0; i--)
            {
                ...
                VectorParameterExpression total = new ConstantExpression(0);
        ...
        private static Expression Differentiate(Function function, Expression variable, BlockExpressionBuilder builder)
        {
            ...
            if (expression is BinaryExpression)
            {
                ...
                if (expression.NodeType == ExpressionType.Multiply)
                {
                    ...
                    {
                        return Expression.Multiply(new ConstantExpression(2), variable);
                    }
                ...
                else if (expression.NodeType == ExpressionType.Add)
                {
                    if (binaryExpression.Left == variable && binaryExpression.Right == variable)
                    {
                        return new ConstantExpression(2);
                    }
                    else if (binaryExpression.Left == variable) return new ConstantExpression(1);
                    else if (binaryExpression.Right == variable) return new ConstantExpression(1);
                }
                ...
                else if (expression.NodeType == ExpressionType.Divide)
                {
                    ...
                    if (binaryExpression.Left == variable)
                    {
                        ...
                        if (right.IsScalar) return builder.AddLocalAssignment(1 / right.ScalarValue, new ScaleInverseExpression(binaryExpression.Right as VectorParameterExpression, 1));
                        else return builder.AddLocalAssignment(new ScaleInverseExpression(binaryExpression.Right as VectorParameterExpression, 1));
                    }
            ...
            if (expression is UnaryMathsExpression)
            {
                ...
                if (unaryExpression.UnaryType == UnaryElementWiseOperations.ScaleOffset) 
                {
                    return new ConstantExpression((unaryExpression as ScaleOffsetExpression).Scale);
                }
                else if (unaryExpression.UnaryType == UnaryElementWiseOperations.Negate) return new ConstantExpression(-1);
                else if (unaryExpression.UnaryType == UnaryElementWiseOperations.ScaleInverse) 
                {
                    return builder.AddNegateDivideExpression(function.Parameter, unaryExpression.Operand); // i.e. x/y => x/y * -1/y 
                }
                else if (unaryExpression.UnaryType == UnaryElementWiseOperations.Exp) return function.Parameter;
                else if (unaryExpression.UnaryType == UnaryElementWiseOperations.Log) return builder.AddInverseExpression(unaryExpression.Operand);
                else if (unaryExpression.UnaryType == UnaryElementWiseOperations.SquareRoot) return builder.AddHalfInverseSquareRootExpression(unaryExpression.Operand);
